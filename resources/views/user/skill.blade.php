...
@include ('dashboard.dashboarduser')                 
        <div class="col-xs-12 col-sm-3 col-md-9 col-lg-9">
                    <div class="info_dv">
                         ...
                         <div class="heads"><i class="fa fa-vine">&nbsp;Skill</i>                           
                            <div class="job" style="float:right !important;">

                               <i class="fa fa-plus-circle" aria-hidden="true" title="Add Skill"><a href="{{route ('user.createskill')}}">&nbsp;Add</a></i>
                            </div>
                        </div>
    ...
    <div class="job_content" >
        <table id="skilltable" class="display" style="width:100%">
            ...
            <tbody>
                ...
                <tr class="job_list">
                    <td>{{ ucwords($skills->score)}}</td>
                    <td>{{ ucwords($skills->name)}}</td>
                    <td>
                        <a class="label label-primary" title="Edit" href="{{route('user.editskill', $skills->id)}}" role="button">EDIT</a>&nbsp;

                        <form id="delete-form-{{$skills->id}}" method="post" action="{{ route('skill.destroy', $skills->id) }}" style="display: none;">
                            @csrf
                            {{ method_field('DELETE')}}
                        </form> 
                            ...
                            if(confirm('Are you sure you want to Delete?'))
                                {
                                    ...
                                    document.getElementById('delete-form-{{ $skills->id}}').submit();
