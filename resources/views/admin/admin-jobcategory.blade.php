...
@include('dashboard.dashboardadmin')               
        <div class="col-xs-12 col-sm-3 col-md-9 col-lg-9">
                    <div class="info_dv">
                        ...
                        <div class="heads"><i class="fa fa-list">&nbsp;Job Categories</i>                           
                            <div class="job" style="float:right !important;">

                                <i class="fa fa-plus"><a href="{{route ('admin.admin-createjobcategory')}}" title="Add Job Category">&nbsp;Add</a></i>
                            </div>
                        </div>
...
@if(count($jobcategories) > 0)
    <div class="job_content" >
        <div class="input-group mb-3">
            {{-- <form action="{{url('admin/job-category/deleteall/')}}" method="POST">
                @csrf --}}
        ...
        <table id="jobcategorytable" class="display" style="width:100%">
            ...
            <tbody>
                ...
                <tr class="job_list">
                    ...
                    <td>{{ ucwords($row->jobcategory)}}</td>
                    <td>{{ Carbon\Carbon::parse($row->created_at)->toFormattedDateString()}}</td>
                    <td>{{ Carbon\Carbon::parse($row->updated_at)->diffForHumans()}}</td>
                    <td>
                        <a class="label label-primary" title="Edit" href="{{route('admin.admin-editjobcategory', $row->id)}}" role="button">EDIT</a>&nbsp;
                        <a href="{{url("admin/job-category/delete/{$row->id}")}}" title="Delete" class="label label-danger" onclick="return confirm('Are you sure you want to delete this item?');">DELETE</a>  
                    </td>
                </tr>
        @endforeach
        </form>
